function LivePage() {
  ...
  const init = () => {
    ...
    focusedSection = "channels"; // Start with channels focused (User Request)
    channelIndex = 0;
    sidebarIndex = 0;
    channelIndex = 0;
    buttonFocusIndex = -1;
  ...
  const render = () => {
    container.innerHTML = `
      <div class="live-header" style="margin-bottom: 60px;">
        <div class="first-live-header">
          <img src="./assets/app-logo.png" alt="Logo" class="setting-header-logo" style="height: 120px; width: auto;"/>
          <div class="live-header-right">
          ${DateTimeComponent()}
          </div>
        </div>
        <div class="second-live-header">
          <p class="live-header-title">Live TV</p>
          <div class="second-live-header-div">
            <div class="live-header-search">
              <input type="text" placeholder="Search Channels" id="live-header-search" class="live-header-search-input"/>
              <img src="./assets/search-icon.png" alt="search" class="live-header-search-icon"/>
            </div>
            <div class="live-header-menu">
  ...
  const updateFocus = () => {
    ...
    const headerMenuIcon = document.querySelector(".live-header-menu");
    if (headerMenuIcon)
    ...
    if (focusedSection === "epg") {
      ...
      const menuIcon = document.querySelector(".live-header-menu");
...
>${timeDisplay}
  ...
  const handleFullscreenChange = () => {
    ...
    // Cross-browser & CSS fullscreen detection
    const isFullscreen =
      checkIsFullscreen() ||
  ...
  const handleKeydown = (e) => {
    ...
    // Cross-browser fullscreen detection extended for back handling
    const playerContainerCheck = document.getElementById("lp-player-container");
    const isCssFullscreen =
      playerContainerCheck &&
      playerContainerCheck.classList.contains("fullscreen-mode");

    const isFullscreenVal =
      checkIsFullscreen() ||
      (window.livePlayer &&
    ...
    if (isBackKey) {
      if (isFullscreenVal) {
        ...
        // Exit CSS fullscreen
        if (isCssFullscreen) {
          playerContainerCheck.classList.remove("fullscreen-mode");
          document.dispatchEvent(new Event("fullscreenchange"));
        }
        return;
      } else {
    ...
    if (isFullscreenVal && e.key === "Enter") {
      e.preventDefault();
  ...
  const navigateDown = () => {
    ...
    if (focusedSection === "header") {
      ...
      if (channelIndex % 3 !== 2 && channelIndex + 1 < filteredStreams.length) {
        ...
        // User Request: Only focus player if video is playing
        if (!isVideoPlaceholderVisible()) {
          if (currentEpgData && currentEpgData.length > 0) {
            focusedSection = "epg";
            epgIndex = 0;
          } else {
  ...
  const navigateLeft = () => {
    ...
    if (focusedSection === "epg") {
      ...
      const allCats = getAllFilteredCategories();
      const sidIdx = allCats.findIndex(
        (c) => String(c.category_id) === String(selectedCategoryId),
      );

      if (sidIdx !== -1) {
        sidebarIndex = sidIdx;
        // Ensure the focused item is loaded
        const neededChunk = Math.ceil((sidIdx + 1) / categoryPageSize);
        if (categoryChunk < neededChunk) {
          categoryChunk = neededChunk;
          renderCategories();
        }
      } else {
        sidebarIndex = 0;
  ...
  const navigateRight = () => {
    ...
    if (focusedSection === "sidebar" || focusedSection === "sidebarSearch") {
      ...
      if (nextIndex < filteredStreams.length) {
        ...
        // User Request: If last item, focus player only if video is playing
        if (!isVideoPlaceholderVisible()) {
          focusedSection = "player";
  ...
  const selectCategory = (newCategoryId) => {
    ...
    if (String(selectedCategoryId) !== String(newCategoryId)) {
      ...
      buttonFocusIndex = -1;
      focusedSection = "channels"; // Focus first item of selected category
