function ParentalControl(restoredValues) {
  ...
  setTimeout(function () {
    ...
    if (restoredValues) {
      inputs[0].value =
        restoredValues && restoredValues[0] ? restoredValues[0] : "";
      inputs[1].value =
        restoredValues && restoredValues[1] ? restoredValues[1] : "";
    ...
    saveButton.addEventListener("click", function () {
      ...
      if (validatePasswords()) {
        ...
        if (currentPlaylist && currentPlaylist.playlistName) {
          updatePlaylistData(
          ...
          console.error("Current playlist not found or has no name");
          Toaster.showToast(
            "error",
            "Could not find active playlist. Please try again.",
          );
          return;
        }
    ...
    clearButton.addEventListener("click", function () {
      ...
      if (
        ...
        if (typeof ParentalPinDialog === "function") {
          ParentalPinDialog(
            performClear,
            () => {
              console.log("Incorrect PIN, clear aborted");
            },
            currentPlaylist,
            "settings",
          );
        } else {
          console.error("ParentalPinDialog not available");
          performClear();
        }
      } else {
        performClear();
